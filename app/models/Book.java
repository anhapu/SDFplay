...
public class Book extends Model {
	...
	public static List<Book> findAllTradeableBooksByTitle(final String title) {
		return find.where().eq("tradeable", true)
	...
	/**
	 * Returns the showcase for a specific user. In the showcase are only the
	 * tradeable books of a user.
	 * 
	 * @param user
	 * @return List of books
	 */
	public static List<Book> getShowcaseForUser(final User user) {
		return find.where().eq("tradeable", true).eq("owner.id", user.id)
				.findList();
	}
